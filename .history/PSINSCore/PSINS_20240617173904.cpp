...
void CKalman::ZmmPkSet(int i)
{
	...
	if(pmm->flag) {  //!如果∣Zk[i]∣ 持续偏大，则增大Pk[i,i] if conitnously big abs(Zk[i]), enlarge Pk[i,i]
		if( pmm->minRes>Zmm0.dd[i] || pmm->maxRes<-Zmm0.dd[i] )
			Pset.dd[Zmmpk[i]] = pmm->meanRes*pmm->meanRes;
	}
}
/**
 * @brief 状态向量、方差阵限制（使-Xmax <=Xk<= Xmax、Pmin<=diag(Pk)<=Pmax）
 * 
 */
...
/**
 * @brief 时间分片组合滤波器更新（含时间和量测更新：把滤波更新分散成2*(nq+nr)+3个时间片/步，
 *                              在每个惯导更新周期内需保证nStep步能运行完；
 *                              每步大约运行0.5ms，但不同处理器间存在差异）
 * 
 * @param pwm 角增量
 * @param pvm 速度增量
 * @param nSamples 字样数
...
//				if((iter-2*Ft.row)%2==0)
				if(iter%2==0) 
				{
					...
					double innoDiff = innovation - innoPre.dd[row];  innoPre.dd[row] = innovation;//!计算新息差值，更新上一次新息
					int *pinnoMaxcount=&innoMaxcount[row]; 
					if(*pinnoMaxcount>0) (*pinnoMaxcount)--; //!更新 pinnoMaxcount，如果创新量超出阈值范围则增加计数
					
                    if(innovation<-innoMax.dd[row] || innovation>innoMax.dd[row])//!新息如果超出阈值
					{
...
/**
 * @brief Construct a new CAlignsv::CAlignsv object
 * 
 * @param pos 初始对准的地理位置
 * @param ts 采样时间间隔
 * @param T2 精对准时间 10<T1<T2/2 default:300
 * @param T1 粗对准时间 default:0
 */
...
void CConUart::sendUart(void)
{
	...
	if(c=='l') {
		...
		else { sprintf(&cmdstr[1], "�����ļ�pbcommand.txt��ȡ����"); }
	}
	else if(cmd[0]=='l' && c=='s') {
		cmd[0] = 'L';  cmd[1] = 's';
